...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
            ...
            while (threadrun)
            {
                ...
                try
                {
                    ...
                    if (tracklast.AddSeconds(Settings.Instance.GetDouble("FD_MapUpdateDelay", 0.3)) < DateTime.Now)
                    {
                        ...
                        if (waypoints.AddSeconds(5) < DateTime.Now)
                        {
                            ...
                            if (wps.Count >= 1)
                            {
                                var homeplla = new PointLatLngAlt(MainV2.ComPort.MAV.cs.HomeLocation.Lat,
                                    ...
                                    MainV2.ComPort.MAV.cs.HomeLocation.Alt / CurrentState.multiplieralt, "Home");
                        ...
                        {
                            addMissionRouteMarker(new GMarkerGoogle(currentloc, GMarkerGoogleType.home)
                            {
                                        ...
                                        Encoding.ASCII.GetString(msg.callsign).Trim('\0') == "OA_DB")
                                    {
                                        ...
                                        if ( ( ( DateTime ) plla.Time ) > DateTime.Now.AddSeconds( -10 ) )
                                            addMissionRouteMarker(
                                                new GMapMarkerDistance( plla, radius, 0 )
                        ...
                        prop.connected = IsConnected;
                        
                        prop.Update(MainV2.ComPort.MAV.cs.MovingBase, MainV2.ComPort.MAV.cs.battery_remaining, ECircleItem.Uav);
        ...
        public Dictionary<string, string> ParamsDict { get; set; }
        ...
        private void tabStatus_Paint(object sender, PaintEventArgs e)
        {
            ...
            foreach (var field in list)
            {
                ...
                var value = cs != null ? typeof( CurrentState ).GetProperty( field ).GetValue( cs )?.ToString( ) : "";

                if (cs != null)
                    g.DrawString(value, this.Font,
                        br, new RectangleF(x + 95, y, 50, 15));

                ParamsDict.Add( field, value );

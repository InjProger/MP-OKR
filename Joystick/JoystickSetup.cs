...
namespace MissionPlanner.Joystick
{
    public partial class JoystickSetup : MyUserControl, IDeactivate
    {
        ...
        JoystickAxis[] _joystickAxises;
        HorizontalProgressBar[] _buttonBars;

        private void Init ()
        {
            try
            {
                CMB_joysticks.Items.AddRange( JoystickWorker.GetJoyNames().ToArray() );
            }
            catch
            {
                CustomMessageBox.Show( "Not connect joystick" );
                return;
            }
            ...
            _joystickAxises = new JoystickAxis[ maxaxis + 1 ];
            ...
            try
            {
                if (MainV2.Joystick == null || MainV2.Joystick.enabled == false)
                {
                    ...
                    if (!joy.start(CMB_joysticks.Text))
                    {
                        ...
                        var text = resourceManager.GetString( "mbNoConnectJoystick.Text", CultureInfo.CurrentUICulture );
                        var caption = resourceManager.GetString( "mbNoConnectJoystick.Caption" );
            ...
            try
            {
                if ( MainV2.Joystick == null || MainV2.Joystick.enabled == false )
                {
                    ...
                    if (joy == null)
                    {
                        ...
                        _buttonBars = new HorizontalProgressBar[ noButtons ];

                        for (int f = 0; f < noButtons; f++)
                        {   
                            string name = (f).ToString();

                            doButtontoUI(name, maxctl.Right + 100, maxctl.Top + f * maxctl.Height, f);
        ...
        void doButtontoUI(string name, int x, int y, int i)
        {
        ...
        {
            var state = j.GetState( );
            tbRoll.Value = state.X;
            tbPitch.Value = state.Z + tbPitch.Minimum;
            tbVelocity.Value = state.Y;
            tbYaw.Value = state.RotationX;
            tbYawMin.Value = state.RotationY;
            tbGimble.Value = state.RotationZ;
            tbFlightMode.Value = state.ViewPoints[ 6 ];
            tbCameras.Value = state.ViewPoints[ 7 ];

            pictureF1.Click = state.Buttons[ 19 ];
            pictureF2.Click = state.Buttons[ 17 ];
            pictureFuse1.Click = state.Buttons[ 7 ];
            pictureFuse2.Click = state.Buttons[ 9 ];
            pictureFire.Click = state.Buttons[ 11 ] || state.Buttons[ 13 ];
        }

        private void JoystickSetup_Load ( object sender, EventArgs e )
        {
            j = new SDL.Joysticks.Joystick( 0 );

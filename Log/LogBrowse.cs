        ...
        private void LogBrowse_Load ( object sender, EventArgs e )
        {
            ...
            if ( !File.Exists( logfilename ) )
            {
                using ( OpenFileDialog openFileDialog1 = new OpenFileDialog( ) )
                {
                    ...
                    var fileSelect = new FileSelect( EOpenFileItem.BlackBox );
                    var dir = lastLogDir = Settings.Instance.LogDir + pathSeparator
                          + "QUADROTOR" + pathSeparator + "1";
                    ...
                    fileSelect.ShowFiles( dir, "bin" );
                    fileSelect.ShowDialog( );

                    var file = fileSelect.SelectedFile;

                    if ( fileSelect.DialogResult == DialogResult.OK )
                    {
                        lastLogDir = Path.GetDirectoryName( file );
                        logfilename = file;
                    ...
                    foreach (var item in gpscache)
                    {
                        ...
                        else if (item.msgtype == "CMD")
                        {
                            ...
                            if (ans != null && ans.Lat != 0 && ans.Lng != 0)
                            {
                                ...
                                mapoverlay.Markers.Add(new GMapMarkerWP(ans, GMarkerGoogleType.green, item["CNum"]));

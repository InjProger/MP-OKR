...
namespace GMap.NET.WindowsForms.Markers
{
   ...
   public class GMarkerGoogle : GMapMarker
#endif
    {
        ...
        public GMarkerGoogle(PointLatLng p, GMarkerGoogleType type) : base(p)
        {
            this._type = type;
        ...
        void LoadBitmap()
        {
            Bitmap = GetIcon(_type.ToString());
            lock (Bitmap)
                Size = new System.Drawing.Size(Bitmap.Width, Bitmap.Height);

            switch (_type)
            {
                case GMarkerGoogleType.home:
                case GMarkerGoogleType.selectFrame:
                case GMarkerGoogleType.landing_place:
                case GMarkerGoogleType.targetCircle:
                    Offset = new Point(-Bitmap.Width / 2, -Bitmap.Height / 2);
                    break;
        ...
        protected GMarkerGoogle(SerializationInfo info, StreamingContext context)
           : base(info, context)
        {
            this._type = Extensions.GetStruct<GMarkerGoogleType>(info, "type", GMarkerGoogleType.none);

...
namespace MissionPlanner.ArduPilot
{
    public class WPOverlay
    {
        ...
        public void CreateOverlay(PointLatLngAlt home, List<Locationwp> missionitems, double wpradius, double loiterradius)
        {
            ...
            var alt = Math.Round(srtm.getAltitude(home.Lat, home.Lng).alt, 0);

            if (home != PointLatLngAlt.Zero)
            {
                home.Tag = "landing_place";
                pointlist.Add(home);
                fullpointlist.Add(pointlist[pointlist.Count - 1]);
                
                addpolygonmarker( "landing_place", home.Lng, home.Lat, home.Alt, null, 0, gAlt:alt);
            }
            ...
            foreach (var itemtuple in missionitems.PrevNowNext())
            {
                    ...
                    || command == (ushort) MAVLink.MAV_CMD.DO_SET_ROI)
                {
                    ...
                    else
                    {
                        ...
                        alt = Math.Round( srtm.getAltitude(item.lat, item.lng).alt, 0);

                        addpolygonmarker((a + 1).ToString(), item.lng, item.lat, item.alt, null, wpradius, gAlt: alt);
                    }
        ...
        private void addpolygonmarker(string tag, double lng, double lat, double? alt, Color? color, double wpradius, MAVLink.MAV_MISSION_TYPE type = MAVLink.MAV_MISSION_TYPE.MISSION, Color? fillcolor = null, double gAlt = 0)
        {
            try
            {
                PointLatLng point = new PointLatLng(lat, lng, gAlt);
                GMapMarker m = null;                
                if(type == MAVLink.MAV_MISSION_TYPE.MISSION)
                {
                    m = new GMapMarkerWP(point, GMarkerGoogleType.green, tag);
                    if (alt.HasValue)
                    {
                ...
                if (tag == "landing_place" )
                {
                    (m as GMarkerGoogle).Type = GMarkerGoogleType.landing_place;
                }

                if ( tag == "Home" )
                {
                    ( m as GMarkerGoogle ).Type = GMarkerGoogleType.home;
                }

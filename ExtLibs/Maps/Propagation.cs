...
namespace MissionPlanner.Maps
{
    ...
    public class Propagation: IDisposable
    {
        ...
        public PointLatLngAlt MovingBase { get; set; } = PointLatLngAlt.Zero;
        ...
        public void Update ( PointLatLngAlt Location, double r, ECircleItem eCircleItem )
        {
            switch ( eCircleItem )
            {
                case ECircleItem.StartPlace:
                    distance.Markers.Add( LittleCircle( ) );
                    distance.Markers.Add( BigCircle( ) );
                    break;
                case ECircleItem.Uav:
                    distance.Markers.Add( LittleCircle( ) );
                    distance.Markers.Add( BigCircle( ) );
                    break;
                case ECircleItem.Point:
                    distance.Markers.Add( BigCircle( ) );
                    break;
                case ECircleItem.Select:
                    distance.Markers.Add( SelectCircle() );
                    break;
            }

            GMapMarkerDistance BigCircle ()
            {
                var markerDistance = new GMapMarkerDistance( Location, r, Settings.Instance.GetFloat( "Propagation_Tolerance" ), true );
                
                markerDistance.IsVisible = true;
                markerDistance.Brush2 = new SolidBrush( Color.Red );
                markerDistance.Pen = new Pen( Brushes.DarkRed, 3.5f );
                markerDistance.Pen2 = new Pen( Brushes.Orange, 5 );

                return markerDistance;
            }

            GMapMarkerDistance LittleCircle()
            {
                var markerDistance = new GMapMarkerDistance( Location, r / 2, Settings.Instance.GetFloat( "Propagation_Tolerance" ) );
                var pen = new Pen( Brushes.Violet, 3.5f );

                markerDistance.IsVisible = true;
                markerDistance.Brush.Color = Color.DarkCyan;
                pen.DashStyle = DashStyle.Dash;
                markerDistance.Pen = pen;
                markerDistance.Pen2 = new Pen( Brushes.Blue, 5 );

                return markerDistance;
            }

            GMapMarkerDistance SelectCircle ( )
            {
                var markerDistance = new GMapMarkerDistance( Location, r, Settings.Instance.GetFloat( "Propagation_Tolerance" ), true );

                markerDistance.IsVisible = true;
                markerDistance.Pen = new Pen( Brushes.Purple, 10 );
            ...
            while (ele_enabled)
            {
                try
                {
                    ...
                    if (rf_run && connected)
                    {
                        ...
                        if (prev_home != MovingBase || need_rf_redraw ||
                            prev_range != Settings.Instance.GetFloat("Propagation_Range") || prev_alt2 != alt)
                        {
                            ...
                            new SightGen(MovingBase, pointslist, MovingBase.Alt, DroneLocation, altasl);
